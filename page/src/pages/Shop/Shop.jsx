      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mt-24 border-t border-white/10 pt-10">
          ...
          <div className="mt-8 grid gap-3">
            {items.length === 0 ? <p className="border border-white/10 bg-white/[0.02] p-8 text-sm text-white/40">Your cart is empty. Add something to get started.</p> : items.map((item) => <div key={item.key} className="flex flex-col gap-4 border border-white/10 bg-white/[0.02] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[9px] uppercase tracking-[0.25em] text-[#9caf88]">{item.type === 'producto' ? 'product' : item.type === 'servicio' ? 'service' : item.type}</p><p className="mt-2 text-base text-white/80">{item.nombre}</p><p className="mt-1 text-xs text-white/35">Maximum available: {item.stock}</p></div><div className="flex items-center gap-5"><div className="flex items-center border border-white/10"><button type="button" onClick={() => updateQuantity(item.key, item.cantidad - 1)} className="px-3 py-2 text-white/50 hover:text-white">-</button><span className="min-w-8 text-center text-sm">{item.cantidad}</span><button type="button" disabled={item.cantidad >= item.stock} onClick={() => updateQuantity(item.key, item.cantidad + 1)} className="px-3 py-2 text-white/50 hover:text-white disabled:cursor-not-allowed disabled:text-white/15">+</button></div><p className="min-w-28 text-right text-sm text-[#c9d5bd]">{formatPrice(Number(item.precio) * item.cantidad)}</p><button type="button" onClick={() => removeItem(item.key)} className="text-xs text-white/30 hover:text-red-300">Remove</button></div></div>)}
          </div>
          {items.length > 0 && <div className="mt-8 flex flex-col items-start justify-between gap-5 border-t border-white/10 pt-6 sm:flex-row sm:items-center"><div><p className="text-[9px] uppercase tracking-[0.25em] text-white/30">Estimated total</p><p className="mt-2 text-2xl font-light text-[#c9d5bd]">{formatPrice(total)}</p></div><button type="button" onClick={handleCheckout} className="border border-[#9caf88] px-6 py-3 text-[10px] uppercase tracking-[0.25em] text-[#9caf88] transition hover:bg-[#9caf88] hover:text-[#07100b]">Continue to checkout</button></div>}
        </section>
      </div>
    </main>
  );
